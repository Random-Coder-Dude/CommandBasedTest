...
public class SwerveHeadingCorrector {
  ...
  private LinearFilter m_gyroVelFilter =
      LinearFilter.singlePoleIIR(Constants.kLoopTime * 5, Constants.kLoopTime);
  ...
  private static final double OMEGA_THRESH = 0.03;

  public ChassisSpeeds update(
      ChassisSpeeds target, ChassisSpeeds cur_vel, Rotation2d gyro, double gyro_vel) {
    ...
    boolean is_near_zero =
        m_yawZeroDetector.update(Math.abs(target.omegaRadiansPerSecond) < OMEGA_THRESH, 0.2);
    ...
    boolean auto_reset = Math.abs(target.omegaRadiansPerSecond) > OMEGA_THRESH || is_rotating;
    ...
    if (is_near_zero && yaw_setpoint.isEmpty()) {
      ...
      m_retSpeeds.omegaRadiansPerSecond =
          m_controller.calculate(current_rotation, yaw_setpoint.get());
      return m_retSpeeds;
    }
